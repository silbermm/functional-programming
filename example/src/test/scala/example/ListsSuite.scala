...
class ListsSuite extends FunSuite {
  ...
  test("list is empty,exception ") {
    intercept[java.util.NoSuchElementException] {
      max(List())
    }
  }
  
  test("list is all zeros, sum is 0"){
    assert(sum(List(0,0,0,0)) === 0)
  }
  
  test("list is all zeros, max is 0"){
    assert(max(List(0,0,0,0)) == 0)
  }
  
  test("list with repeats, max is correct"){
    assert(max(List(2,2,0)) === 2)
  }
}
